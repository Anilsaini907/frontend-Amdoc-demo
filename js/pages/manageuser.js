...
function showUseraddupdateToast(message, isSuccess) {
  var toastBody = $("#addupdateToast .toast-body");
  toastBody.text(message);

  var toast = $("#addupdateToast");
  if (isSuccess) {
    toastBody.removeClass("text-danger").addClass("text-success");
  } else {
    toastBody.removeClass("text-success").addClass("text-danger");
...
function editUser() {
  ...
  $.ajax({
    ...
    success: function (response) {
      //console.log("response", response);
      if (response.status === "true") {
        showUseraddupdateToast("User Updated Sucessfully!", true);
        setTimeout(function () {
          window.location.reload();
        }, 2000);
      }
...
$(document).ready(function () {
  ...
  var table = $("#example1").DataTable({
    ...
    columns: [
      ...
      },
      {
        ...
        data: null,
        defaultContent:
          "<button  class='edit btn btn-warning' data-toggle='modal' data-target='#loginModal' title='update role'><i class='mdi mdi-account-box'></i></button><button style='margin-left:10px;' class='editpass btn btn-warning' data-toggle='modal' data-target='#updateModal' title='update password'><i class='mdi mdi-pencil-box-outline'></i></button><button style='margin-left:10px;' class='delete btn btn-danger' data-toggle='modal' data-target='#deleteuser' title='delete'><i class='mdi mdi-delete'></i></button>",
      },
    ],
  });
  ...
  $("#example1 tbody").on("click", ".edit", function () {
    ...
    if (buttonDisplayed) {
      const inputElement = document.getElementById("useremail");
      const div = document.getElementById("addToast");
      div.setAttribute("id", "addupdateToast");
      // console.log("div",div);
